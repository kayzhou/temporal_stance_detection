...
class NeighborSampler(BlockSampler):
    ...
    def sample_neighbors(self, g, seed_nodes):
        seed_nodes = dgl.utils.prepare_tensor(g, seed_nodes, 'seed_nodes')  #将seed_nodes列表转变成一个适合图的张量
        self.restart_prob = F.copy_to(self.restart_prob, F.context(seed_nodes))  #将restart_prob放在和seed_nodes相同设备上（如GPU）方便计算
        ...
        src_2 = F.reshape(paths[:, self.random_walk_length::self.random_walk_length], (-1,)) #将形状变成一维张量
        ...
        mask_2 = (src_2 != -1)
        mask = (src != -1)
        src = src[mask] #掩码  删除无效节点和边  这可能是由于数据录入错误导致的
        dst = dst[mask]
        eids = eids[mask_2]
        ...
        # neighbor_graph.edata['hete'] = torch.cat([torch.ones(etype_1.shape[0]), torch.zeros(etype_2.shape[0])])
        # neighbor_graph.edata['homo'] = torch.cat([torch.zeros(etype_1.shape[0]), torch.ones(etype_2.shape[0])])
        neighbor_graph.edata['first_etime'] = torch.cat([etime[:, 0], etime[:, 0]], dim=0)  #所有边的第一次交互时间
        neighbor_graph.edata['second_etime'] = torch.cat([etime[:, 1], etime[:, 1]], dim=0) #所有边的第二次交互时间
...
def generate_user(user, data, graph, item_max_length, user_max_length, train_path, test_path,val_path=None):
    ...
    test_num = 0
    # 生成训练数据
    if len(u_seq) < 10: #最少交互十次
        return 0, 0 #如果用户序列小于3 则不要这个用户信息了
    else:#用户有足够多的邻居 len(time) >=10
        test_t = random.sample(list(u_time[8:]), 1)[0]
        for j, t  in enumerate(u_time[0:-1]): #对于每一个时间戳（已经排好序了） 取历史时间戳
            try:
                if j < 8:  #j的取值就是user邻居节点的个数 j=8 第九次交互开始
                    continue
                ...
                sub_u_eid_1 = (graph.edges['postby'].data['time'] < u_time[j]+1) & (graph.edges['postby'].data['time'] >= start_t)
                sub_u_eid_2 = (graph.edges['post'].data['time'] < u_time[j]+1) & (graph.edges['post'].data['time'] >= start_t)
                sub_i_eid_1 = (graph.edges['retweetby'].data['time'] < u_time[j]+1) & (graph.edges['retweetby'].data['time'] >= start_t)
                sub_i_eid_2 = (graph.edges['retweet'].data['time'] < u_time[j]+1) & (graph.edges['retweet'].data['time'] >= start_t)
                sub_graph = dgl.edge_subgraph(graph, edges = {'postby':sub_u_eid_1, 'post':sub_u_eid_2,'retweetby':sub_i_eid_1,'retweet':sub_i_eid_2}, relabel_nodes=False)
                homo_g = dgl.to_homogeneous(sub_graph,edata=['time'],return_count=True)[0]

                num_texts = len(data)
                u_temp = torch.tensor([user + num_texts])



                sampler = NeighborSampler(random_walk_length=2,num_neighbors=3,num_layers=2,num_random_walks=10)
                seed_nodes, output_nodes, blocks_1 = sampler.sample_blocks(g=homo_g,seed_nodes=u_temp)
                        
                u_t_labels = u_labels[:j+1]
                ...
                'dst_node_1':output_nodes,
                'dst_node_0':seed_nodes} #第一个block和第二个block的目标节点
                ...
                print(f'Error processing user: {str(user)}_{str(j)}')
                print(f'Error message:{str(e)}')    
        return train_num, test_num
    
    
def generate_data(data, graph, item_max_length, user_max_length, train_path, test_path, val_path):
    user = data['user_id'].unique()
    all_num = []
    for u in user:
        a = generate_user(u, data, graph, item_max_length, user_max_length, train_path, test_path, val_path)
        all_num.append(a)
        print(f"用户{u}处理完毕")
    return all_num        
...
if __name__ == '__main__':
    ...
    graph_path = '/home/kayzhou/yuexutong/DGSR-master/DGCR-Dubleh/DGCR_0725/hete_g0724_dglsave_graph'
    ...
    train_path = f'Newdata/stance_blocknew0731_{opt.item_max_length}_{opt.user_max_length}_{opt.k_hop}/train'
    val_path = f'Newdata/stance_blocknew0731_{opt.item_max_length}_{opt.user_max_length}_{opt.k_hop}/val'
    test_path = f'Newdata/stance_blocknew0731_{opt.item_max_length}_{opt.user_max_length}_{opt.k_hop}/test'
